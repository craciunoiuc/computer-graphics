...
void Laborator5::Update(float deltaTimeSeconds)
{
...

	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::scale(modelMatrix, glm::vec3(2, 1, 2));
		modelMatrix = glm::rotate(modelMatrix, RADIANS(60.0f), glm::vec3(1, 1, 1));
		modelMatrix = glm::translate(modelMatrix, glm::vec3(8, 0.5f, 3));
		modelMatrix = glm::rotate(modelMatrix, RADIANS(50.0f), glm::vec3(1, 1, 0));
		RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
	}
...
static float fov = 0;
static float inaltime = 0;
static float latime = 0;
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_O)) {
		projectionMatrix = glm::ortho(-3.0f + latime, 3.0f + latime, -3.0f + inaltime, 3.0f + inaltime, 0.01f, 200.0f);
	}
