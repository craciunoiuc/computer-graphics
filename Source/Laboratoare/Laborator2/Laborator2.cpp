...
void Laborator2::Init()
{
...

	{
		vector<VertexFormat> vertices
		{
			VertexFormat(glm::vec3(0.5, 1,  0), glm::vec3(0.5, 1, 0)),
			VertexFormat(glm::vec3(1, 0,  0), glm::vec3(1, 0, 0)),
			VertexFormat(glm::vec3(0, 0,  0), glm::vec3(0, 0, 0)),
			VertexFormat(glm::vec3(0.5, 0.5,  1), glm::vec3(0.5, 0.5, 1)),
			// TODO: Complete the information for the cube
		};
		...
		// Create a new mesh from buffer data
		Mesh* cube = CreateMesh("tetrahedron", vertices, indices);
	}

	{
		...
		// Create a new mesh from buffer data
		Mesh* cube = CreateMesh("tetragon", vertices, indices);
	}
}
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(-2.5f, 1.5f, 1), glm::vec3(0.25f));

	RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(-3.5f, 2.5f, 1), glm::vec3(0.25f));
	RenderMesh(meshes["tetragon"], shaders["VertexColor"], glm::vec3(1.0f, 2.0f, 1), glm::vec3(0.25f));
