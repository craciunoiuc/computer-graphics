...
{
	rot += deltaTimeSeconds * 10;
	trX += deltaTimeSeconds * 2;
	modelMatrix = visMatrix * Transform2D::Scale(3, 0.5); // Corp
	//modelMatrix *= Transform2D::Translate(trX, 1);
	RenderMesh2D(meshes["square1"], shaders["VertexColor"], modelMatrix);


	modelMatrix = visMatrix * Transform2D::Translate(1, 0.5); // Cap
	//modelMatrix *= Transform2D::Translate(trX, 1);
	RenderMesh2D(meshes["square1"], shaders["VertexColor"], modelMatrix);


	modelMatrix = visMatrix * Transform2D::Scale(0.2, 0.2) * Transform2D::Translate(0, -0.5); // Roata stanga
	modelMatrix *= Transform2D::Rotate(rot);
	//modelMatrix *= Transform2D::Translate(trX, 1);
	RenderMesh2D(meshes["square1"], shaders["VertexColor"], modelMatrix);


	modelMatrix = visMatrix * Transform2D::Scale(0.2, 0.2) * Transform2D::Translate(14, -0.5); // Roata dreapta
	modelMatrix *= Transform2D::Rotate(rot);
	//modelMatrix *= Transform2D::Translate(trX, 1);
	RenderMesh2D(meshes["square1"], shaders["VertexColor"], modelMatrix);
